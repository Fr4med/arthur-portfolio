...
The homepage begins with a separate black 3D section. The original header, two-column introduction, film cover, archive, About section and footer follow it unchanged.

## Animation

components/camera-scroll-intro.tsx measures native page scroll through a 320svh section with a sticky 100svh viewport. The scrollable distance drives a normalized 0-1 progress value. There is no scroll interception, autoplay, inertia or drag rotation in this introduction. Stopping the scroll stops the model. Scrolling back reverses the same sequence.
...
- Hold the exact front-facing pose for the remaining 18% before the sticky section releases and the original site enters.
...
The Enter Site link skips to the original header. Reduced-motion visitors see the final still pose in a single-height section. Without JavaScript the intro is hidden. Failed loading or a lost WebGL context shows the supplied render in a single-height section, leaving the site accessible below.
...
Run node --experimental-strip-types --test tests/camera.test.mjs to verify asset integrity, coordinate mapping, complete rotation, final hold, reverse scroll mapping and framing. Build with npm run build.
