...
The site uses the SHABBAT SESH blue, #659fd5, as its shared accent. Brand colors are defined in app/globals.css and apply to every route, contact/social sections and interaction states. The favicon uses the same blue. Cool charcoal backgrounds support the palette; the paper artwork retains its original colors. The original opening section is restored from pre-threejs-redesign: the two-column introduction, paper film cover, FILM IT. sticker and film links. The Three.js component is retained in the project but is not mounted on the homepage while the animation flow is being defined.
...
The retained Three.js prototype can load the supplied Panasonic HMC150 GLB from public/models. It is currently inactive on the website. The original editable Blender file remains in ../outputs/panasonic-hmc150. The scene adds turntable and floating movement at runtime; the model contains no animation clips. Visitors can drag horizontally, rotate with arrow keys, pause/play, or reset the view. Reduced-motion visitors start with a still model. Rendering stops off-screen and when the tab is hidden. If loading or WebGL fails, the supplied model render is shown. Other routes, films, the filtered Instagram feed and the contact form retain their existing React implementation. See THREEJS.md for implementation and rollback details.
