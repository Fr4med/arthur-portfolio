...
The site uses the SHABBAT SESH blue, #659fd5, as its shared accent. Brand colors are defined in app/globals.css and apply to every route, contact/social sections and interaction states. The favicon uses the same blue. Cool charcoal backgrounds support the palette; the paper artwork retains its original colors. The original opening section is restored from pre-threejs-redesign: the two-column introduction, paper film cover, FILM IT. sticker and film links. A separate scroll-driven Three.js introduction now precedes the original header and opening section.
...
The first section loads the supplied Panasonic HMC150 GLB from public/models. Scroll progress moves it down from above, rotates it through 360 degrees in the horizontal plane and tilts it 90 degrees from lens-up to lens-toward-viewer. The final pose holds before the original header and site scroll into view. Scrolling backward reverses the sequence. The intro uses native sticky positioning and does not intercept scrolling. Reduced-motion visitors see the final still pose in a single-height section. The Enter Site link skips to the original header; without JavaScript the intro is hidden. A model-load or WebGL failure shows a one-screen preview. See THREEJS.md for coordinate mapping, timing and rollback details.
